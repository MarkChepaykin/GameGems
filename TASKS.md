...
### TASK-CC16 · Порталы на поле
**Приоритет: высокий | ~5-6 часов**

`cell.portal={id,exitR,exitC}`. В `explodeCell()` → спавнить гем у exit. STRIPE/BOMB продолжают движение от exit-клетки. Визуал: два кольца одного цвета + дуга. Генератор: difficulty>70 → 1-2 пары. Хардкодные L90-L112.

**AC:** Гем у входа → появляется у выхода. STRIPE проходит сквозь портал.
